...
async def transition_case(
    ...
    # 1. Fetch case with Parcel to resolve Project ID
    result = await db.execute(select(AcquisitionCase).where(AcquisitionCase.id == case_id))
    case = result.scalars().first()
    if not case:
        raise HTTPException(status_code=404, detail="Case not found")
    ...
    # 2. Authorization Checks (BOLA)
    AuthorizationService.verify_mutation_access(current_user)
    AuthorizationService.verify_project_access(current_user, str(parcel.project_id))
...
async def get_case_deadline(case_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(AcquisitionCase).where(AcquisitionCase.id == case_id))
    case = result.scalars().first()
    if not case:
        raise HTTPException(status_code=404, detail="Case not found")
...
async def get_case_audit(case_id: UUID, db: AsyncSession = Depends(get_db)):
