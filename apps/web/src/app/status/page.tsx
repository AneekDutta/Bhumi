...
export default async function StatusPage() {
  ...
  const subsystems = [
    { name: 'Decision-Intelligence API Gateway', icon: Server, type: 'FastAPI / ASGI Uvicorn', status: isHealthy ? 'OPERATIONAL' : 'DEGRADED', latency: '34ms', uptime: '99.98%', color: '#0a2c5f', details: 'Processes counterfactual simulations, statutory stage validations, and project portfolio feeds.' },
  ...
  const statutoryEngines = [
    ...
    {
      act: 'National Highways Act 1956', title: 'Central Highway Alignment & Land Acquisition', rulesCount: '9 Active Rules', color: '#0a2c5f',
  ...
  const archTiers = [
    { tier: 'Tier 1: Presentation', name: 'Next.js 15 App Router', desc: 'Tailwind CSS, MapLibre GL geospatial layers, Lucide icons, responsive command portal.', detail: 'Port: 3000 · SSR & CSR', color: '#0a2c5f' },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 10, fontFamily: 'JetBrains Mono, monospace', padding: '3px 10px', borderRadius: 5, background: '#f1f5f9', border: '1px solid rgba(255,255,255,0.08)', color: '#94a3b8' }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {[
            ...
            { label: 'Data Integrity', val: '100% Signed', color: '#0a2c5f' },
          ].map(s => (
            <div key={s.label} style={{ padding: '12px 16px', borderRadius: 10, background: '#f1f5f9', border: '1px solid rgba(255,255,255,0.08)' }}>
              <div style={{ fontSize: 9, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', fontFamily: 'JetBrains Mono, monospace' }}>{s.label}</div>
        ...
        {subsystems.map((sub) => {
          ...
          return (
            <div key={sub.name} className="glass tr-hover" style={{ borderRadius: 14, padding: '20px', borderTop: `3px solid ${sub.color}` }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, fontFamily: 'JetBrains Mono, monospace', color: '#94a3b8', marginTop: 1 }}>{sub.type}</div>
                  </div>
                </div>
              ...
              <p style={{ fontSize: 11, color: '#94a3b8', lineHeight: 1.6, marginBottom: 12 }}>{sub.details}</p>
              <div style={{ paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: '#94a3b8' }}>
                <span>Ping: <strong style={{ color: '#475569' }}>{sub.latency}</strong></span>
      ...
      <div className="glass" style={{ borderRadius: 14, padding: '20px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 8 }}>
          <div>
            <div style={{ fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: '#94a3b8', letterSpacing: '0.07em', textTransform: 'uppercase' }}>Statutory Compliance</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', marginTop: 2 }}>Ruleset Engine</div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 14 }}>
          {statutoryEngines.map((eng) => (
            <div key={eng.act} style={{ padding: '16px 20px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 12 }}>{eng.title}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                {eng.rules.map((r) => (
                  <div key={r} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: '#475569', lineHeight: 1.4 }}>{r}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="glass" style={{ borderRadius: 14, padding: '20px 24px' }}>
        <div style={{ fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: '#94a3b8', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 4 }}>BHUMI Decision Platform</div>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', marginBottom: 16 }}>Architecture Topology</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
          {archTiers.map((t) => (
            <div key={t.tier} style={{ padding: '16px 18px', borderRadius: 10, background: `${t.color}0a`, border: `1px solid ${t.color}28` }}>
              ...
              <p style={{ fontSize: 11, color: '#94a3b8', margin: '0 0 8px', lineHeight: 1.5 }}>{t.desc}</p>
        ...
        <div style={{ marginTop: 14, padding: '12px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: 9, border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Terminal style={{ width: 13, height: 13, color: '#94a3b8', flexShrink: 0 }} />
            <span style={{ fontSize: 11, color: '#94a3b8' }}>Core Daemon: <strong style={{ fontFamily: 'JetBrains Mono, monospace', color: '#475569' }}>bhumi-chronos-daemon (Active)</strong></span>
          </div>
          <span style={{ fontSize: 10, fontFamily: 'JetBrains Mono, monospace', color: '#94a3b8' }}>Lapse Check: 12h interval · Next in 4h 12m</span>
        </div>
      </div>
    </div>
  );
