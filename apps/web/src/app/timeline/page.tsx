...
const getPipelineStages = () => {
  ...
  return [
    { stage: 'SIA / Sec 4', cases: countStage('SIA'), limit: '6 months', color: '#0a2c5f', pct: calcPct(countStage('SIA')) },
...
const STAGE_STYLE: Record<string, { bg: string; color: string }> = {
  PRELIMINARY_NOTIFICATION: { bg: 'rgba(99,102,241,0.12)', color: '#0a2c5f' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))', gap: 14 }}>
        {[
          { label: 'Total Cases', val: totalCases, color: '#0a2c5f', bg: 'rgba(99,102,241,0.1)', border: 'rgba(99,102,241,0.22)' },
      ...
      <div className="glass" style={{ borderRadius: 14, padding: '20px 24px' }}>
        <div style={{ fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: '#94a3b8', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 4 }}>Stage Funnel</div>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', marginBottom: 20 }}>Cases per RFCTLARR Stage</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {pipelineStages.map((s) => (
            <div key={s.stage}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: '#1e293b' }}>{s.stage}</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 10, fontFamily: 'JetBrains Mono, monospace', color: '#94a3b8' }}>Limit: {s.limit}</span>
      ...
      <div className="glass" style={{ borderRadius: 14, padding: '20px 24px' }}>
        <div style={{ fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: '#94a3b8', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 4 }}>Statutory Framework</div>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', marginBottom: 16 }}>RFCTLARR Time Limit Reference</div>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: '#f8fafc' }}>
                {['Section', 'Trigger Event', 'Time Limit', 'Consequence on Lapse', 'Severity'].map(h => (
                  <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 9, fontFamily: 'JetBrains Mono, monospace', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {RFCTLARR_LIMITS.map((r) => {
                ...
                return (
                  <tr key={r.section} className="tr-hover" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    <td style={{ padding: '11px 14px', fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, color: '#0a2c5f' }}>{r.section}</td>
                    <td style={{ padding: '11px 14px', color: '#475569' }}>{r.trigger}</td>
                    <td style={{ padding: '11px 14px', fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, color: '#1e293b' }}>{r.limit}</td>
                    <td style={{ padding: '11px 14px', color: '#94a3b8', fontSize: 11 }}>{r.consequence}</td>
      ...
      <div className="glass" style={{ borderRadius: 14, padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: '#94a3b8', letterSpacing: '0.07em', textTransform: 'uppercase' }}>Acquisition Cases</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', marginTop: 2 }}>Filterable Case Register</div>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
            ...
            <tbody>
              {MOCK_CASES.length === 0 ? (
                ...
                MOCK_CASES.map((c) => {
                  const stageStyle = STAGE_STYLE[c.stage] || { bg: 'rgba(99,102,241,0.1)', color: '#0a2c5f' };
                  ...
                  return (
                    <tr key={c.id} className="tr-hover" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      <td style={{ padding: '11px 16px' }}>
                        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, color: '#0a2c5f' }}>{c.survey_no}</span>
                      </td>
                      <td style={{ padding: '11px 16px', color: '#475569', fontSize: 11 }}>{c.project}</td>
                      <td style={{ padding: '11px 16px' }}>
                      ...
                      <td style={{ padding: '11px 16px', color: '#94a3b8', fontSize: 11 }}>{c.owner}</td>
                      <td style={{ padding: '11px 16px' }}>
                        {c.lapsed ? (
                          ...
                          <span style={{ fontSize: 9, color: '#94a3b8', fontFamily: 'JetBrains Mono, monospace' }}>—</span>
                        )}
                      </td>
                      <td style={{ padding: '11px 16px' }}>
                        <Link href={projHref} style={{ fontSize: 11, fontWeight: 600, color: '#94a3b8', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
